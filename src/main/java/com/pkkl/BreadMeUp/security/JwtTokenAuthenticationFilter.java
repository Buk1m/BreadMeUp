...
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        String header = request.getHeader(jwtAuthSettings.getHeader());

        if (header == null || !header.startsWith(jwtAuthSettings.getPrefix())) {
            chain.doFilter(request, response);
            return;
        }

        String token = header.replace(jwtAuthSettings.getPrefix() + ' ', "");

        try {
            Claims claims = Jwts.parser()
                    .setSigningKey(jwtAuthSettings.getSecret().getBytes())
